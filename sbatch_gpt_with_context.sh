...
mkdir /tmp/${USER}/hf_cache/${SLURM_ARRAY_TASK_ID}_wc_cache
mkdir /tmp/${USER}/hf_cache/${SLURM_ARRAY_TASK_ID}_wc_cache/transformer
mkdir /tmp/${USER}/hf_cache/${SLURM_ARRAY_TASK_ID}_wc_cache/hf
conda activate /tudelft.net/staff-umbrella/tunoMSc2023/codes/gpt_env/
export TRANSFORMERS_CACHE=/tmp/${USER}/hf_cache/${SLURM_ARRAY_TASK_ID}_wc_cache/transformer
export HF_HOME=/tmp/${USER}/hf_cache/${SLURM_ARRAY_TASK_ID}_wc_cache/hf
#srun matlab < matlab_script.m # Computations should be started with 'srun'.
python /tudelft.net/staff-umbrella/tunoMSc2023/codes_repo/label_array_process_with_context.py
rm -r /tmp/${USER}/hf_cache/${SLURM_ARRAY_TASK_ID}_wc_cache
